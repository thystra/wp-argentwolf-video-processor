...
Readiness statistics are deliberately disposable operational telemetry. Record a sample from staged-upload `accepted_at` through `verified_at` (remote `ready_verified`), not through later public cutover, so finalizer retries, administrator delay, and recovery do not inflate the backend estimate. The Overview may summarize the estimator's exact size buckets and let administrators reset one backend or all readiness samples. Resetting statistics must never clear task/event/upload journals, remote assets, publication-health history, or serving authority.

At the outbound publication/send boundary, require a current provider catalog before freezing the manifest or beginning the upload. A non-stale catalog may be reused for at most 300 seconds; older catalogs are refreshed after credential repair. If a current catalog cannot be established, keep the publication sync in a recoverable authority wait before any file bytes are sent. Finalizer and processing polls reuse valid generation-bound catalog state and do not force five-minute refresh traffic while PeerTube transcodes.
