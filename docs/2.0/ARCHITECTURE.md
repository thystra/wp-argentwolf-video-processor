...
RC13.2 adds two explicit administrator recovery paths without weakening those boundaries. **Check now** invokes the existing visitor-facing serving-health probe for one exact remote asset and persists the observation through the normal health/event repositories; it has no upload, republish, privacy/publication mutation, or frontend-network authority. If that fresh observation is healthy but unattended recovery is still held by the two-success hysteresis, **Restore remote serving now** may mark only that exact unchanged healthy row serving-eligible and records the operator actor/time. Any changed asset/health timestamp or failed/indeterminate probe refuses the shortcut.

**Rebuild local delivery** is the sole deliberate exception to ordinary destination-based local-processing suppression. It is available only while the retained WordPress source remains exact and retention cleanup does not fence processing. The web request first persists an exact source/profile authorization, then queues the existing local job and launches the detached worker; it never runs FFmpeg inline. The worker must re-prove that authorization after atomically claiming the job before it may process a video whose configured primary destination is remote. The rebuild adds/replaces local delivery evidence only; it does not change the configured destination, publication lifecycle, remote assets, or historical serving audit evidence.
