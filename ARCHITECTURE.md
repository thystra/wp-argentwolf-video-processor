...
The RC13.10 application boundary now accepts a supported pasted URL and creates or reuses one durable AWVP Video. PeerTube-shaped URLs are positively verified through the instance public `/api/v1/videos/{id}` endpoint before creation; only Public privacy is accepted, and aliases such as short UUID or numeric lookup forms converge to the authoritative UUID returned by PeerTube. A configured PeerTube backend is not required for public embedding. If the verified instance origin and UUID already match an AWVP-managed remote asset on any configured PeerTube backend (including retired descriptors), the existing AWVP Video is reused instead of creating an external duplicate. Database lookup uncertainty fails indeterminate rather than being treated as absence.

External video state uses dedicated provider-neutral source metadata and `source_state=external`; it carries no WordPress attachment and no publishing destination. The existing editor state can therefore represent local, retired-remote-only, and external AWVP Videos through one object model. External state cannot be assigned a publishing destination through the editor boundary, does not create serving authority, and is never retention/archive proof. Local retention rejects `external` before policy mutation and rechecks the state in the detached worker so a cleanup queued earlier cannot delete bytes after the record becomes external.

The completed RC13.10 authoring/rendering boundary keeps the block serialized state to one stable AWVP Video ID. An unbound block can choose an existing usable AWVP Video, bind WordPress Media Library video, or paste a supported public provider URL. The existing-video list includes local, verified remote-only, and external records that the current editor may modify. External records render from their sanitized durable identity only; visitor requests perform no provider HTTP. PeerTube, YouTube, and Vimeo use one responsive iframe contract with lazy loading, a strict-origin referrer policy, fullscreen/picture-in-picture only, and no autoplay permission. Managed PeerTube serving continues to use its independently verified serving authority rather than external-source metadata.
...
### Explicit serving and local-delivery recovery

RC13.2 adds **Check now** for one exact broken/held remote publication. It runs the existing visitor-facing health probe without changing PeerTube publication state; after a fresh healthy result, **Restore remote serving now** may satisfy the recovery hysteresis only while that exact health observation still matches. **Rebuild local delivery** is separate: it freezes the retained WordPress source and HLS-capable profile, queues the existing local processing job, and permits the detached worker to bypass remote-destination suppression only for that exact authorization. It does not retarget the video or erase remote publication history.

RC13.2 also exposes **History & Logs** as a read-only projection of the bounded event store; it never becomes a second task replay path. Scheduled **Send now** publication has a separate reviewed pre-publication visibility: Private by default, optionally Unlisted only while the anchor is actually `future`. That staging target is generation-fenced, cannot establish serving authority, and is replaced by the reviewed final privacy only after the real WordPress `publish` transition.

### Local retention at scale

WordPress is the archive of record by default, and original-source deletion is prohibited while that policy is active. Serving choice and source retention are separate. Local Retention provides one site-wide server default plus a searchable/filterable compact video list for selected-video policy assignment. A cleanup delay of `0` means Never automatically clean up; selected videos may snapshot a finite 1-365 day delay without rewriting the server default. **Remove local copies now** is a separate one-time command rather than a delay value: once accepted it persists a queued state, the detached worker moves through running/completion, and every destructive safety check is repeated before media changes occur.

Source-prune/keep-HLS keeps the WordPress attachment because the retained local delivery remains attachment-scoped and removes only the exact source through WordPress's `wp_delete_file()` boundary. Delete-all is different: after verified remote serving, archive authority, exact source identity, exclusive AWVP ownership, and direct WordPress-reference checks all pass, AWVP records a durable source-retirement tombstone and retires the Media Library attachment through `wp_delete_attachment()`. The AWVP Video remains as the durable remote-only identity with serving authority and history. Direct Core Video/legacy/raw-source references, or an incomplete post-reference scan, fail closed before source retirement. No Media Library record is deleted with raw SQL and no WordPress source is unlinked outside the reviewed WordPress lifecycle boundaries.
