...
## RC13.9 completed-retirement reconciliation regression

The RC13.7/RC13.8 migration path must not reuse the active-cleanup full authority hash after source deletion has already completed. Routine health verification legitimately advances serving-authority `verified_at`. Completed attachment retirement may ignore that mutable timestamp only when backend ID, serving generation, plan SHA-256, manifest SHA-256, remote asset ID, and remote UUID still match the completed cleanup journal and current remote serving remains verified. Active source deletion must continue using the exact full proof hash. Tests must include both timestamp-only refresh (retirement succeeds) and durable identity drift (retirement fails closed).

## RC13.11 external-authoring scope-reduction regression

RC13.11 must prove the generic external-video application boundary is absent while preserving the managed PeerTube remote-only renderer. Tests should require no `/editor/external-videos` route, no shipped YouTube/Vimeo/public-PeerTube external-ingestion runtime classes, and no external URL authoring UI. The reusable AWVP Video picker must continue to include usable local and managed remote-only records. The carried-forward RC13.9 livefix qualification must continue proving source-retired managed PeerTube rendering, safe iframe attributes, and no autoplay.
