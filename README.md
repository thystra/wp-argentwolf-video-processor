...
Local Retention reports the effective **Server default** or per-video **Override** instead of `Not set`. A destructive zero-day policy means **Manual cleanup only**. The bulk list has an independent policy selector, and **Remove local copies now** explicitly applies zero-day remove-all behavior to selected videos without changing the server default. Archive-of-record, serving-proof, processing, source-identity, policy, and WordPress filesystem-confinement checks remain mandatory in the detached worker. Fresh/unconfigured installations default to zero retention days; an explicitly saved existing nonzero value is not rewritten.
