...
$GLOBALS['r469_deleted_attachments']=array();
$GLOBALS['r469_direct_attachment_refs']=array();
$GLOBALS['r469_reference_truncated']=false;
...
function get_post(int $id):mixed{if(isset($GLOBALS['r469_video_status'][$id]))return(object)array('ID'=>$id,'post_type'=>Video_Post_Type::POST_TYPE,'post_status'=>$GLOBALS['r469_video_status'][$id]);if(in_array($id,array(20,21),true)&&!in_array($id,$GLOBALS['r469_deleted_attachments'],true))return(object)array('ID'=>$id,'post_type'=>'attachment','post_status'=>'inherit','post_title'=>'Video '.$id);return null;}
function get_post_mime_type(int $id):string|false{return is_object(get_post($id))&&'attachment'===(get_post($id)->post_type??null)?'video/mp4':false;}
function get_the_title(int $id):string{return 'Video '.$id;}
function wp_delete_attachment(int $id,bool $force=false):mixed{unset($force);$post=get_post($id);if(!is_object($post)||'attachment'!==($post->post_type??null))return false;$GLOBALS['r469_deleted_attachments'][]=$id;$GLOBALS['r469_source_absent']=true;unset($GLOBALS['r469_outputs'][$id],$GLOBALS['r469_meta'][$id]);return $post;}
...
final class Video_Meta{
 public const MASTER_AUTHORITY='master';public const SOURCE_STATE='source';public const SOURCE_TOMBSTONE='source_tombstone';public const CLEANUP_STATE='cleanup';public const LOCAL_RETENTION_POLICY='policy';public const LOCAL_RETENTION_EXECUTION='execution';public const ATTACHMENT_ID='attachment';public const SERVING_AUTHORITY='authority';public const DESTINATION='destination';
...
final class WordPress_Source_File{
 ...
 public static function identity_absent(array $v):bool{unset($v);return (bool)$GLOBALS['r469_source_absent'];}
...
final class Source_Retirement_Record{
 public static function capture(int $attachment_id,array $source,int $task_id,int $removed_at=0):array{if($attachment_id<1||$task_id<1||$removed_at<0||array()===$source||!is_object(get_post($attachment_id)))return array();return array('version'=>1,'former_attachment_id'=>$attachment_id,'attachment_title'=>'Video '.$attachment_id,'filename'=>'video-'.$attachment_id.'.mp4','mime_type'=>'video/mp4','relative_path'=>$source['relative_path'],'bytes'=>$source['bytes'],'source_identity'=>$source,'retention_task_id'=>$task_id,'removed_at'=>$removed_at);}
 public static function sanitize(mixed $v):array{return is_array($v)&&1===($v['version']??0)&&isset($v['former_attachment_id'],$v['source_identity'],$v['retention_task_id'],$v['removed_at'])?$v:array();}
 public static function with_removed_at(array $v,int $now):array{$v=self::sanitize($v);if(array()===$v||$now<1)return array();$v['removed_at']=$now;return $v;}
}
final class Video_Reference_Index{
 public function truncated():bool{return (bool)$GLOBALS['r469_reference_truncated'];}
 public function attachment_posts_for(int $attachment_id):array{return $GLOBALS['r469_direct_attachment_refs'][$attachment_id]??array();}
}
...
use ArgentVideo\{Local_Retention_Service as S,Task_Repository,Job_Repository,FakeServing,Video_Meta,Local_Retention_Policy as P,Archive_Of_Record_Policy_Store,Video_Reference_Index};
...
$tasks=new Task_Repository();$jobs=new Job_Repository();$serving=new FakeServing();$references=new Video_Reference_Index();$svc=new S($tasks,$serving,$jobs,null,$references);
$siteTasks=new Task_Repository();$siteJobs=new Job_Repository();$sitePolicy=new Archive_Of_Record_Policy_Store();$siteSvc=new S($siteTasks,$serving,$siteJobs,$sitePolicy,$references);
$GLOBALS['r469_video_refs']=array(100);
...
$beforeRemoveNow=count($siteTasks->enqueues);$removeNow=$siteSvc->remove_local_copies_now(100,7,1666);$removePolicy=P::sanitize($GLOBALS['r469_meta'][100][Video_Meta::LOCAL_RETENTION_POLICY]??array());$removeQueue=end($siteTasks->enqueues);$a(S::APPLIED===$removeNow['status']&&1666===$removeNow['eligible_at']&&count($siteTasks->enqueues)===$beforeRemoveNow+1&&P::MODE_DELETE_ALL===($removePolicy['mode']??'')&&0===($removePolicy['grace_days']??-1)&&'backend_source'===($GLOBALS['r469_meta'][100][Video_Meta::MASTER_AUTHORITY]??'')&&'queued'===($GLOBALS['r469_meta'][100][Video_Meta::CLEANUP_STATE]??'')&&true===($removeQueue['payload']['manual_cleanup']??false),'Explicit remove-local-copies action did not override Keep/default semantics and queue an immediate guarded delete-all task.');
$GLOBALS['r469_meta'][100][Video_Meta::CLEANUP_STATE]='none';unset($GLOBALS['r469_meta'][100][Video_Meta::LOCAL_RETENTION_EXECUTION]);$GLOBALS['r469_meta'][100][Video_Meta::SOURCE_STATE]='present';$GLOBALS['r469_direct_attachment_refs'][20]=array(array('id'=>999,'title'=>'Direct core/video ref'));$blockedByReference=$siteSvc->remove_local_copies_now(100,7,1667);$a(S::REFUSED===$blockedByReference['status'],'Direct non-AWVP attachment reference did not fail closed before source retirement.');unset($GLOBALS['r469_direct_attachment_refs'][20]);
...
$GLOBALS['r469_meta'][100][Video_Meta::CLEANUP_STATE]='none';$GLOBALS['r469_meta'][100][Video_Meta::SOURCE_STATE]='present';$GLOBALS['r469_meta'][100][Video_Meta::ATTACHMENT_ID]=20;$GLOBALS['r469_outputs'][20]=array('mp4'=>array('path'=>'x'));$GLOBALS['r469_source_absent']=false;$GLOBALS['r469_source_matches']=true;$beforeDeletes=$GLOBALS['r469_source_deleted'];$r=$svc->configure(100,P::MODE_DELETE_ALL,1,'backend_source',7,200000);$q=end($tasks->enqueues);$task=array('id'=>$r['task_id'],'video_post_id'=>100,'task_type'=>S::TASK_TYPE,'lock_token'=>'33333333-3333-4333-8333-333333333333','payload_json'=>json_encode($q['payload']));$w=$svc->advance_claimed($task,$r['eligible_at']);$a(S::STATUS_COMPLETE===$w['status']&&$GLOBALS['r469_source_deleted']===$beforeDeletes&&'removed'===($GLOBALS['r469_meta'][100][Video_Meta::SOURCE_STATE]??''),'Delete-all retention must retire the source through the WordPress attachment lifecycle rather than the standalone source-file delete boundary.');$a(null===ArgentVideo\get_post(20),'Delete-all retention did not retire the WordPress attachment through the attachment lifecycle.');$a(!isset($GLOBALS['r469_meta'][100][Video_Meta::ATTACHMENT_ID]),'Retired attachment binding remained on the durable AWVP Video.');$a(array()!==ArgentVideo\Source_Retirement_Record::sanitize($GLOBALS['r469_meta'][100][Video_Meta::SOURCE_TOMBSTONE]??array()),'Source retirement tombstone was not persisted before attachment retirement.');
// RC13.7 migration: a completed delete-all journal with already-absent source retires the stale Media Library attachment on detached recovery.
$GLOBALS['r469_video_refs']=array(101);$GLOBALS['r469_source_absent']=true;$GLOBALS['r469_source_matches']=false;$GLOBALS['r469_meta'][101][Video_Meta::SOURCE_STATE]='removed';$GLOBALS['r469_meta'][101][Video_Meta::ATTACHMENT_ID]=21;$GLOBALS['r469_meta'][101][Video_Meta::MASTER_AUTHORITY]='backend_source';$GLOBALS['r469_meta'][101][Video_Meta::SERVING_AUTHORITY]=$auth;$migrationPolicy=P::create(P::MODE_DELETE_ALL,0,7,210000);$GLOBALS['r469_meta'][101][Video_Meta::LOCAL_RETENTION_POLICY]=$migrationPolicy;$migrationExecution=ArgentVideo\Local_Retention_Execution::create_remote(101,21,$migrationPolicy,$auth,ArgentVideo\WordPress_Source_File::capture(21),1,210000,210000);$migrationExecution=ArgentVideo\Local_Retention_Execution::with_task($migrationExecution,77);$migrationExecution=ArgentVideo\Local_Retention_Execution::transition($migrationExecution,ArgentVideo\Local_Retention_Execution::STATUS_COMPLETE,210001);$GLOBALS['r469_meta'][101][Video_Meta::LOCAL_RETENTION_EXECUTION]=$migrationExecution;$GLOBALS['r469_meta'][101][Video_Meta::CLEANUP_STATE]='complete';$siteSvc->reconcile_completed_retirements();$a(null===ArgentVideo\get_post(21),'Detached completed-cleanup reconciliation did not retire the stale RC13.7 attachment.');$a(!isset($GLOBALS['r469_meta'][101][Video_Meta::ATTACHMENT_ID]),'Completed-cleanup reconciliation did not clear stale attachment binding.');$a(array()!==ArgentVideo\Source_Retirement_Record::sanitize($GLOBALS['r469_meta'][101][Video_Meta::SOURCE_TOMBSTONE]??array()),'Completed-cleanup reconciliation did not preserve a source tombstone.');
