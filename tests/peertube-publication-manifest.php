...
$pre=$plan;$pre['pre_publish_privacy_id']=Plan::PRE_PUBLISH_UNLISTED;$a([]!==Manifest::build($pre,$catalog,$settings),'Reviewed Unlisted pre-publication visibility was not accepted when the provider supports it.');
$precat=$catalog;unset($precat['privacies'][Plan::PRE_PUBLISH_UNLISTED]);$a([]===Manifest::build($pre,$precat,$settings),'Unlisted pre-publication visibility was accepted when the provider catalog does not support it.');
