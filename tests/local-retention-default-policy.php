...
final class Local_Retention_Policy{public const MODE_KEEP='keep',MODE_DELETE_MANAGED='delete_managed',MODE_DELETE_SOURCE_KEEP_DELIVERY='delete_source_keep_delivery',MODE_DELETE_ALL='delete_all';}
}
...
$r=$store->save('delete_source_keep_delivery',7,1002);$assert(Store::APPLIED===($r['status']??'')&&'delete_source_keep_delivery'===$store->mode(),'Source-prune/local-delivery default policy was not representable.');
$r=$store->save('delete_all',7,1003);$assert(Store::APPLIED===($r['status']??'')&&'delete_all'===$store->mode(),'Delete-all default policy was not representable for a site that allows it at the admin boundary.');
$before=$GLOBALS['awvp_retention_default_options'];$r=$store->save('bogus',7,1004);$assert(Store::REFUSED===($r['status']??'')&&$before===$GLOBALS['awvp_retention_default_options'],'Malformed default policy mutated durable state.');
