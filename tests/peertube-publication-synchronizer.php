...
function wp_is_post_revision(int|object $post): int|false { $id=is_object($post)?(int)($post->ID??0):(int)$post; $p=get_post($id); return is_object($p)&&'revision'===($p->post_type??'')?(int)($p->post_parent??0):false; }
function wp_is_post_autosave(int|object $post): int|false { return false; }
...
$GLOBALS['awvp_sync_posts'][11]=(object)array('ID'=>11,'post_parent'=>10,'post_type'=>'revision','post_status'=>'inherit','post_content'=>'x');
$GLOBALS['awvp_sync_posts'][12]=(object)array('ID'=>12,'post_type'=>'post','post_status'=>'publish','post_content'=>'x');
...
$before_life=$life; $before_tasks=count($GLOBALS['wpdb']->rows); $sync->transition('inherit','publish',$GLOBALS['awvp_sync_posts'][11]); $assert($before_life===$GLOBALS['awvp_sync_meta'][100][Video_Meta::PEERTUBE_PUBLICATION_LIFECYCLE]&&$before_tasks===count($GLOBALS['wpdb']->rows),'Revision inherit transition superseded the published anchor lifecycle.');
$sync->transition('publish','draft',$GLOBALS['awvp_sync_posts'][12]); $assert($before_life===$GLOBALS['awvp_sync_meta'][100][Video_Meta::PEERTUBE_PUBLICATION_LIFECYCLE]&&$before_tasks===count($GLOBALS['wpdb']->rows),'Reused block on a non-anchor post acquired publication transition authority.');
...
$GLOBALS['awvp_sync_posts'][101]=(object)array('ID'=>101,'post_type'=>'argent_video_asset','post_status'=>'draft');
$stagePlan=$plan; $stagePlan['pre_publish_privacy_id']=Plan::PRE_PUBLISH_UNLISTED;
$GLOBALS['awvp_sync_meta'][101]=array(Video_Meta::ORIGIN_POST_ID=>10,Video_Meta::DESTINATION=>array('version'=>1,'backend_id'=>'pt-primary','channel_id'=>'41'),Video_Meta::PEERTUBE_PUBLICATION_PLAN=>$stagePlan);
$r=$sync->sync_video(101,'future',1042); $stageLife=$GLOBALS['awvp_sync_meta'][101][Video_Meta::PEERTUBE_PUBLICATION_LIFECYCLE];
$assert(Task_Repository::APPLIED===$r['status']&&true===$stageLife['upload_authorized']&&false===$stageLife['reveal_authorized']&&Plan::PRE_PUBLISH_UNLISTED===$stageLife['target_privacy_id'],'Scheduled Send-now video did not stage at reviewed Unlisted visibility.');
$r=$sync->sync_video(101,'publish',1043); $stageLife=$GLOBALS['awvp_sync_meta'][101][Video_Meta::PEERTUBE_PUBLICATION_LIFECYCLE];
$assert(2===$stageLife['generation']&&true===$stageLife['reveal_authorized']&&'1'===$stageLife['target_privacy_id'],'Actual WordPress publish did not advance staged Unlisted visibility to reviewed final privacy.');
$before=count($GLOBALS['wpdb']->rows); $r=$sync->sync_republish_video(100,'draft',1050,6); $life=$GLOBALS['awvp_sync_meta'][100][Video_Meta::PEERTUBE_PUBLICATION_LIFECYCLE]; $assert(Task_Repository::APPLIED===$r['status']&&6===$r['generation']&&6===$life['generation'],'Explicit republish did not advance to its exact target generation.');
$after=count($GLOBALS['wpdb']->rows); $assert($after===$before+1,'Explicit republish did not create exactly one new lifecycle task.');
$r=$sync->sync_republish_video(100,'draft',1051,6); $assert(Task_Repository::PRESENT===$r['status']&&$after===count($GLOBALS['wpdb']->rows),'Republish replay duplicated the exact-generation task.');
$r=$sync->sync_republish_video(100,'draft',1052,8); $assert(Task_Repository::CONFLICT===$r['status']&&6===$r['generation'],'Republish was allowed to skip a lifecycle generation.');
...
$assert(1===substr_count($plugin,'add_action(Activator::CRON_HOOK,')&&str_contains($plugin,"add_action('argentwolf_video_processor_task_enqueued'"),'RC9 publication tasks are not event-driven while local FFmpeg remains on the five-minute cron.');
