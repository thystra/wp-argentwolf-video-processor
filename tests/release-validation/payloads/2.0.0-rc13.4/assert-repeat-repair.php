...
// indexes, force the model version stale, then prove dbDelta restores schema 3
...
$health_indexes = awvp_release_indexes($health);
awvp_release_assert(isset($health_indexes['next_check']), 'Publication-health next_check index missing before repair fixture.');
$wpdb->query("ALTER TABLE `{$health}` DROP INDEX `next_check`");
$health_indexes = awvp_release_indexes($health);
awvp_release_assert(! isset($health_indexes['next_check']), 'Could not remove publication-health next_check index for repair fixture.');
...
awvp_release_assert(isset($health_indexes['next_check']), 'Model schema repair did not restore publication-health next_check index.');
awvp_release_assert(0 === (int) $wpdb->get_var("SELECT COUNT(*) FROM `{$health}`"), 'Model schema repair created publication-health rows.');
