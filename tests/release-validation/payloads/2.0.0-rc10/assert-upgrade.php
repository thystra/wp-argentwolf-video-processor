...
$legacy_discovery = new \ArgentVideo\Legacy_Video_Adoption_Service();
$legacy_inspection = $legacy_discovery->inspect($attachment_id);
awvp_release_assert(
    'eligible' === ($legacy_inspection['status'] ?? null)
    && $post_id === (int) ($legacy_inspection['anchor_post_id'] ?? 0),
    'The preserved 1.0 Core Video fixture is not discoverable as an eligible legacy migration candidate after upgrade.'
);
$legacy_candidates = $legacy_discovery->candidates(20, 0);
$legacy_keys = array_map(
    static fn (array $row): string => is_string($row['candidate_key'] ?? null) ? $row['candidate_key'] : '',
    $legacy_candidates['items']
);
awvp_release_assert(
    in_array('legacy:' . $attachment_id, $legacy_keys, true),
    'Read-only legacy migration discovery did not include the preserved 1.0 fixture.'
);
awvp_release_assert(
    ! metadata_exists('post', $attachment_id, \ArgentVideo\Legacy_Video_Adoption_Service::ATTACHMENT_ASSET_META),
    'Read-only legacy migration discovery adopted the 1.0 fixture.'
);
