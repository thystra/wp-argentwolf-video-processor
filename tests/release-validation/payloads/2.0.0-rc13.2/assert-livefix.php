...
foreach ($required_classes as $class) {
    awvp_release_assert(class_exists($class) || interface_exists($class), "Required RC13.2 live-fix runtime class/interface is not packaged: {$class}");
}
...
awvp_release_assert(
    ...
    'RC13.2 processing viability state is not available.'
);
...
awvp_release_assert(
    ...
    'Unconfigured remote backend serving priority did not use the safe RC13.2 default.'
);
...
awvp_release_assert(
    ...
    'RC13.2 safe indeterminate-upload operator-retirement state contract is not packaged.'
);
awvp_release_assert(
    'argentwolf_video_processor_overview_resolve_upload' === \ArgentVideo\PeerTube_Overview_Admin::ACTION_RESOLVE_UPLOAD,
    'RC13.2 Overview indeterminate-upload resolution action is not packaged.'
);
awvp_release_assert(
    'argentwolf_video_processor_overview_reset_readiness' === \ArgentVideo\PeerTube_Overview_Admin::ACTION_RESET_READINESS,
    'RC13.2 Overview readiness-statistics reset action is not packaged.'
);
...
awvp_release_assert(
    ...
    'RC13.2 missing/terminal publication-finalizer recovery contract is not packaged.'
);
awvp_release_assert(
    method_exists(\ArgentVideo\PeerTube_Publication_Task_Coordinator::class, 'finalize_idempotency_key'),
    'RC13.2 exact-generation finalizer idempotency contract is not packaged.'
);
...
awvp_release_assert(
    ...
    'RC13.2 local-retention Never/source-prune policy contract is not packaged.'
);
...
awvp_release_assert(
    ...
    'RC13.2 zero-day Never and keep-local-delivery retention semantics are unavailable.'
);
awvp_release_assert(
    ...
    'RC13.2 Videos & Routing / History & Logs operator surfaces are not packaged.'
);
awvp_release_assert(
    ...
    'RC13.2 explicit remote-health/local-rebuild operator actions are not packaged.'
);
awvp_release_assert(
    ...
    'RC13.2 recovery audit/request metadata is not registered for AWVP Video posts.'
);
awvp_release_assert(
    ...
    'RC13.2 explicit pre-publication Private/Unlisted visibility contract is not packaged.'
);
awvp_release_assert(
    ...
    'RC13.2 operator inventory/history/recovery runtime methods are unavailable.'
);

echo "AWVP_RC13_2_LIVEFIX_CONTRACT_PASS\n";
