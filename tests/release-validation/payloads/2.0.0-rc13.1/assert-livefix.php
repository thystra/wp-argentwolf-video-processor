...
foreach ($required_classes as $class) {
    awvp_release_assert(class_exists($class) || interface_exists($class), "Required RC13.1 live-fix runtime class/interface is not packaged: {$class}");
}
...
awvp_release_assert(
    ...
    'RC13.1 processing viability state is not available.'
);
...
awvp_release_assert(
    ...
    'Unconfigured remote backend serving priority did not use the safe RC13.1 default.'
);
...
awvp_release_assert(
    ...
    'RC13.1 safe indeterminate-upload operator-retirement state contract is not packaged.'
);
awvp_release_assert(
    'argentwolf_video_processor_overview_resolve_upload' === \ArgentVideo\PeerTube_Overview_Admin::ACTION_RESOLVE_UPLOAD,
    'RC13.1 Overview indeterminate-upload resolution action is not packaged.'
);
awvp_release_assert(
    'argentwolf_video_processor_overview_reset_readiness' === \ArgentVideo\PeerTube_Overview_Admin::ACTION_RESET_READINESS,
    'RC13.1 Overview readiness-statistics reset action is not packaged.'
);
...
awvp_release_assert(
    ...
    'RC13.1 missing/terminal publication-finalizer recovery contract is not packaged.'
);
awvp_release_assert(
    method_exists(\ArgentVideo\PeerTube_Publication_Task_Coordinator::class, 'finalize_idempotency_key'),
    'RC13.1 exact-generation finalizer idempotency contract is not packaged.'
);

echo "AWVP_RC13_1_LIVEFIX_CONTRACT_PASS\n";
