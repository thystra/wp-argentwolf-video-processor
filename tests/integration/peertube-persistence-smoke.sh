#!/usr/bin/env bash
# Real-WordPress/database persistence smoke for the R35 checkpoint.
#
...
FIXTURE_RELATIVE='tests/fixtures/peertube-persistence-smoke/assert-persistence.php'
FIXTURE_ROOT="$REPOSITORY_ROOT/tests/fixtures/peertube-persistence-smoke"
...
RESOURCE_PREFIX="awvp-r35-$RUN_TOKEN"
...
if [[ -n "${AWVP_R35_REPORT_DIR:-}" ]]; then
    if [[ "$AWVP_R35_REPORT_DIR" != /* ]]; then
        echo 'AWVP_R35_REPORT_DIR must be an absolute path.' >&2
        exit 2
    fi
    case "$AWVP_R35_REPORT_DIR/" in
        "$REPOSITORY_ROOT/"*)
            echo 'AWVP_R35_REPORT_DIR must remain outside the repository checkout.' >&2
            exit 2
            ;;
    esac
    mkdir -p -- "$AWVP_R35_REPORT_DIR"
    REPORT_DIRECTORY="$(cd -- "$AWVP_R35_REPORT_DIR" && pwd -P)"
else
    REPORT_DIRECTORY="$(mktemp -d /tmp/awvp-r35-report.XXXXXX)"
fi
...
REPORT_FILE="$REPORT_DIRECTORY/peertube-persistence-smoke-$RUN_TOKEN.log"
touch -- "$REPORT_FILE"
exec > >(tee -a "$REPORT_FILE") 2>&1

fail() {
    echo "PEERTUBE_PERSISTENCE_SMOKE_ERROR=$*" >&2
    exit 1
}
...
cleanup() {
    ...
    echo "PEERTUBE_PERSISTENCE_SMOKE=$RESULT"
...
run_case() {
    ...
    wp_cli core install \
        --url='http://wp' \
        --title="AWVP R35 persistence smoke $CURRENT_CASE" \
    ...
    for forbidden_value in \
        ...
        'r35-access-token-value-two' \
        'r35-refresh-token-value-two' \
        'r35-access-token-must-not-win'; do
...
[[ -f "$FIXTURE_ROOT/assert-persistence.php" ]] || fail 'The persistence assertion fixture is missing.'
...
WORK_DIRECTORY="$(mktemp -d /tmp/awvp-r35-work.XXXXXX)"
...
[[ -f "$EXPORTED_FIXTURE" ]] || fail 'The committed persistence fixture was not exported.'
...
RESULT='PASS'
echo 'PEERTUBE_PERSISTENCE_MATRIX_ASSERTIONS=PASS'
