#!/usr/bin/env bash
# Real-WordPress/database coordinator smoke for the R36 checkpoint.
#
# This exports one exact clean commit and runs it against the oldest and newest
# supported WordPress/PHP/database cases. It is development-checkpoint evidence,
...
FIXTURE_RELATIVE='tests/fixtures/peertube-connection-coordinator-smoke/assert-step.php'
FIXTURE_ROOT="$REPOSITORY_ROOT/tests/fixtures/peertube-connection-coordinator-smoke"
...
RESOURCE_PREFIX="awvp-r36-$RUN_TOKEN"
...
if [[ -n "${AWVP_R36_REPORT_DIR:-}" ]]; then
    if [[ "$AWVP_R36_REPORT_DIR" != /* ]]; then
        echo 'AWVP_R36_REPORT_DIR must be an absolute path.' >&2
        exit 2
    fi
    case "$AWVP_R36_REPORT_DIR/" in
        "$REPOSITORY_ROOT/"*)
            echo 'AWVP_R36_REPORT_DIR must remain outside the repository checkout.' >&2
            exit 2
            ;;
    esac
    mkdir -p -- "$AWVP_R36_REPORT_DIR"
    REPORT_DIRECTORY="$(cd -- "$AWVP_R36_REPORT_DIR" && pwd -P)"
else
    REPORT_DIRECTORY="$(mktemp -d /tmp/awvp-r36-report.XXXXXX)"
fi
...
REPORT_FILE="$REPORT_DIRECTORY/peertube-connection-coordinator-smoke-$RUN_TOKEN.log"
touch -- "$REPORT_FILE"
exec > >(tee -a "$REPORT_FILE") 2>&1

fail() {
    echo "PEERTUBE_CONNECTION_COORDINATOR_SMOKE_ERROR=$*" >&2
    exit 1
}
...
cleanup_case() {
    ...
    for container_spec in \
        ...
        # The pinned images may declare anonymous volumes. -v removes only
        # those after container ownership is established; bind/named mounts
...
cleanup() {
    ...
    echo "PEERTUBE_CONNECTION_COORDINATOR_SMOKE=$RESULT"
...
wp_cli() {
    docker run --rm \
        ...
        -e WORDPRESS_DEBUG=1 \
        -e "AWVP_R36_STEP=$COORDINATOR_STEP" \
...
run_case() {
    ...
    for container_name in "$DATABASE_NAME" "$WORDPRESS_NAME"; do
    ...
    wp_cli core install \
        --url='http://wp' \
        --title="AWVP R36 coordinator smoke $CURRENT_CASE" \
    ...
    if ! wp_cli eval '
        ...
        if ( ! $valid ) {
            fwrite( STDERR, "The coordinator runtime configuration is incomplete.\n" );
            exit( 1 );
        }
    ...
    for coordinator_step in \
        ...
        echo "COORDINATOR_PROCESS_BEGIN=$CURRENT_CASE:$coordinator_step"
        wp_cli --context=cli eval-file \
            "/var/www/html/wp-content/plugins/argentwolf-video-processor/$FIXTURE_RELATIVE" \
            --use-include
        echo "COORDINATOR_PROCESS_END=$CURRENT_CASE:$coordinator_step:PASS"
    done
    COORDINATOR_STEP=''
    echo "COORDINATOR_WP_CLI_PROCESS_COUNT=$CURRENT_CASE:11"
    echo "COORDINATOR_RESTART_SEQUENCE=$CURRENT_CASE:PASS"
    echo "COORDINATOR_SINGLE_TARGET_BOUNDARIES=$CURRENT_CASE:PASS"
    echo "COORDINATOR_SEMANTIC_READY_AFTER_UNRELATED_APPEND=$CURRENT_CASE:PASS"
    echo "COORDINATOR_OCCUPIED_IDENTITY_REFUSAL=$CURRENT_CASE:PASS"
    echo "COORDINATOR_CREDENTIAL_INPUT_REFUSAL=$CURRENT_CASE:PASS"
    echo "COORDINATOR_WORDPRESS_HTTP_REQUESTS=$CURRENT_CASE:NONE"
    echo "COORDINATOR_UPLOAD_MUTATIONS=$CURRENT_CASE:NONE"

    for forbidden_value in \
        'r36-password-canary' \
        'r36-otp-canary' \
        'r36-access-token-canary' \
        'r36-refresh-token-canary'; do
        if grep --fixed-strings --quiet -- "$forbidden_value" "$REPORT_FILE"; then
            fail "A synthetic secret appeared in the preserved report for $CURRENT_CASE."
        fi
        if docker exec "$WORDPRESS_NAME" \
            grep --fixed-strings --quiet -- \
                "$forbidden_value" /var/www/html/wp-content/debug.log \
            >/dev/null 2>&1; then
            fail "A synthetic secret appeared in WP_DEBUG_LOG for $CURRENT_CASE."
        fi
    done
...
for command_name in chmod date docker git grep mkdir mktemp openssl rm seq tar tee; do
    require_command "$command_name"
done

[[ -f "$FIXTURE_ROOT/assert-step.php" ]] || fail 'The coordinator assertion fixture is missing.'
...
WORK_DIRECTORY="$(mktemp -d /tmp/awvp-r36-work.XXXXXX)"
SOURCE_EXPORT="$WORK_DIRECTORY/argentwolf-video-processor"
EXPORTED_FIXTURE="$SOURCE_EXPORT/$FIXTURE_RELATIVE"
...
[[ -f "$EXPORTED_FIXTURE" ]] || fail 'The committed coordinator fixture was not exported.'
...
RESULT='PASS'
echo 'PEERTUBE_CONNECTION_COORDINATOR_MATRIX_ASSERTIONS=PASS'
