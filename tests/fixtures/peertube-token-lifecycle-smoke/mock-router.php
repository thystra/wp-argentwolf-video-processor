...
if ('POST' === $method && '/api/v1/users/token' === $target) {
    ...
    if ('refresh_token' === ($parsed['grant_type'] ?? null)) {
        $expected = array(
            'client_id'     => 'r37-oauth-client-id',
            'client_secret' => 'r37-oauth-client-secret-canary',
        ...
        if (
            ...
            $reject('refresh-form-or-authority');
            return;
        }
        if (! $append_log('POST /api/v1/users/token scenario=refresh-success form=refresh')) {
            $problem(500);
            return;
        }
        $json(
            array(
                'token_type'               => 'Bearer',
                'access_token'             => 'r41-refreshed-access-token-canary',
                'refresh_token'            => 'r41-refreshed-refresh-token-canary',
                'expires_in'               => 3600,
...
if ('POST' === $method && '/api/v1/users/revoke-token' === $target) {
    if (
        'Bearer r41-refreshed-access-token-canary' !== $authorization
        || '' !== $otp
        || '' !== $body
    ) {
        $reject('revoke-authority');
        return;
    }
    if (! $append_log('POST /api/v1/users/revoke-token auth=bearer body=none')) {
        $problem(500);
        return;
    }
    $json(array());
