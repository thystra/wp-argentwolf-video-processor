...
- Extend Local Retention so a cleanup delay of `0` means **Never automatically delete**, and let a selected bulk application snapshot either the site default, Never, or a finite 1-365 day per-video delay without rewriting unrelated videos.
- Add **Remove original; keep verified local HLS delivery copies** as a distinct retention mode. This path does not require PeerTube: it freezes and re-proves the plugin-managed HLS master/rendition/segment evidence, retains the delivery tree and `_argent_video_outputs`, removes only the exact WordPress source file through the existing confined WordPress deletion boundary, and preserves the attachment object.
- Version local-retention execution journals forward while accepting existing version-1 remote-proof journals unchanged, so already queued RC13.1 cleanup evidence remains readable and cannot be reinterpreted under the new local-HLS proof model.
- Add explicit **Check now** remote-serving verification for current incidents and a guarded **Restore remote serving now** action that may waive the unattended two-success recovery delay only after that exact remote asset passes a fresh visitor-facing check. Health checks cannot upload, republish, or change PeerTube publication state, and operator actor/time/result are retained as bounded audit evidence.
- Add explicit **Rebuild local delivery** from a retained WordPress source. The administrator action only prepares and queues the existing FFmpeg/HLS job for the detached worker; the worker must match the frozen source/profile authorization before bypassing ordinary remote-destination routing, and historical remote publication state is preserved.
- Report remote-serving recovery email only after the publication is serving-eligible again, and rename Overview readiness wording to **Estimated readiness**.
- Add a read-only **History & Logs** tab over the bounded durable publication-event store. It groups retained publication activity into compact rows with video/post identity, backend, outcome, current serving source, remote identity/link where safe, and expandable event details; it has no task-resume or provider-mutation controls.
- Add reviewed pre-publication PeerTube visibility for scheduled **Send now** publishing. Private remains the safe default; an explicit Unlisted choice may be applied only while the anchor is actually `future`, never establishes serving authority, and final visibility still waits for the real WordPress `publish` transition. Existing version-1 plans without the new optional field retain their exact plan hash.
- Replace normal operator-event wording such as “durable boundary” and “publication authority” with concrete checks, progress, waits, superseded-task outcomes, and next actions while retaining implementation details in technical diagnostics.
...
- Tighten PeerTube readiness telemetry so estimator samples measure durable upload acceptance through remote `ready_verified`, never later publication/finalizer/operator delay. Add exact size-band readiness summaries, per-backend/reset-all administrator controls that clear advisory estimator samples only, and an Overview backend summary for current serving, healthy remote copies, active work, and attention counts.
- Require a current PeerTube publication catalog at the send boundary: a catalog older than five minutes is refreshed before the publication manifest is frozen or an upload session is opened, and refresh/credential uncertainty fails closed before file transfer rather than discovering removed provider choices after upload.
- Make the downstream GitHub mirror CI manual-only while Forgejo remains authoritative for automatic validation, preventing recurring mirror-only failure mail without removing the diagnostic workflow.
