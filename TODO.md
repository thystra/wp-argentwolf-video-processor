...
- [x] Build/qualify/install canonical `2.0.0-rc13.1` from authoritative RC12 candidate #4. Exact source commit `d27a76f80128bf522a878722b251e40c2c4af957`, tree `a0f6caa9bb767a3279bbeaf1f4ce59bd3e748d89`, and qualified inner ZIP SHA-256 `05f0909fcd4756650a91281f5634552c10d1bc0e6b01ed8139e75319934ed744` are immutable evidence.
- [x] Recover Test 8 on RC13.1 without re-upload/new generation and complete a representative legacy 1.x migration smoke test while preserving historical post content, retained source, no-autoplay rendering, and normal verified PeerTube cutover.
- [x] Complete the RC13.2 source implementation window: Videos & Routing/migration UX, Never/per-video retention plus keep-HLS/source-prune mode, explicit Check now/verified remote restore and retained-source local-delivery rebuild, read-only History & Logs/operator wording cleanup, and reviewed Private/Unlisted pre-publish visibility for scheduled Send-now publishing.
- [x] Freeze canonical RC13.2 at commit `2123d60a32eb9012a836f312bf6d89ef2b4d39ce`, tree `cc070fd771dddf268c12e4d14143e1e9e11680fe`, exact inner ZIP SHA-256 `59898407857cb0576bebbd0c631b481b0ca4ac8fa62a39cbe8ace1ab6543bf57`; exhaustive diagnostics proved all six clean/1.0-upgrade cases pass, but Plugin Check rejects the package on Local Retention nonce-analysis warnings. Preserve RC13.2 as rejected immutable evidence.
- [ ] Build and qualify `2.0.0-rc13.3` with the Local Retention request-boundary correction and aggregate release-validation failure collection, then resume controlled production/live acceptance only from the exact qualified RC13.3 bytes.
- [ ] If defects require code changes, increment the prerelease identity before rebuilding. Prefer `2.0.0-rcN.M` for successive immutable builds within one RC series (`rc13.1`, `rc13.2`, `rc13.3`, ...); advance `N` for the next planned RC tranche. Never mutate or reuse an existing prerelease version/tag.
...
- [x] Extend the block authoring UX with **Choose existing AWVP Video**, **Paste video URL**, and the existing WordPress Media Library path, including remote-only/external AWVP Video selection.
- [x] Render verified external PeerTube/YouTube/Vimeo identities through one provider-neutral iframe path with safe attributes, responsive layout, and no autoplay.
- [x] Update plugin header/README/WordPress.org-facing description now that the external embedding workflow is end to end.
