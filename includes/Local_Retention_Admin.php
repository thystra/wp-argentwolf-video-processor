...
final class Local_Retention_Admin
{
    ...
    public function page():void
    {
        ...
        <?php foreach($rows as $row):$id=(int)$row->ID;$authority=Video_Serving_Authority::sanitize(get_post_meta($id,Video_Meta::SERVING_AUTHORITY,true));$policy=Local_Retention_Policy::sanitize(get_post_meta($id,Video_Meta::LOCAL_RETENTION_POLICY,true));$cleanup=Video_Meta::sanitize_cleanup_state(get_post_meta($id,Video_Meta::CLEANUP_STATE,true));$source=Video_Meta::sanitize_source_state(get_post_meta($id,Video_Meta::SOURCE_STATE,true));$master=Video_Meta::sanitize_master_authority(get_post_meta($id,Video_Meta::MASTER_AUTHORITY,true));?>
        ...
        <?php if('removed'===$source||'complete'===$cleanup):?><strong><?php esc_html_e('Local cleanup complete; irreversible physical cleanup state is frozen.','argentwolf-video-processor'); ?></strong><?php else:?>
